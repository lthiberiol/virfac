...
vfdb_folder = '/work/hgt/bepe/aeromonas/virulence_factors/vfdb'
control_folder = '/work/hgt/bepe/aeromonas/virulence_factors/control/blast'
...
for hg_id in hmm_positives:
    ...
    vfdb_parameters =  {'desc':[], 'scores':[], 'identity':[], 'coverage':[]}
...
for n in all:
    ...
    if n in true_positives:
        ...
        positive_identities.extend(identity)
        positive_coverages.extend(coverage)
    else:
        negative_scores['vfdb'].append(tmp1)
        negative_scores['control'].append(tmp2)
        negative_identities.extend(identity)
        negative_coverages.extend(coverage)
    all_identities.extend(identity)
...
fig, axScatter = pl.subplots()
fig.suptitle("Identities and coverage of putative T3SSs", fontsize=30)
...
axHisty.set_xticks([])
fig.tight_layout()
fig.savefig('ipad_sync/identity_coverage.png')
fig.savefig('ipad_sync/identity_coverage.pdf')
################################################################################
